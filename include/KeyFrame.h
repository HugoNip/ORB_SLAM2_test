...
public:
    ...
    static long unsigned int nNextId;   // 下一个keyframe的序号
    long unsigned int mnId;             // 当前keyframe的序号
    const long unsigned int mnFrameId;  // 生成这个keyframe的Frame id
    ...
    // 此为frame ID，标记是哪个**keyframe**查询过和此keyframe有相同的单词，且在essentialgraph中连接
    long unsigned int mnLoopQuery;
    // mnLoopQuery指向的Frame和此keyframe有多少共同的单词
    int mnLoopWords;
    // 此为通过dbow计算的mnLoopQuery指向的Frame与此keyframe之间**相似度的得分**
    float mLoopScore;

    // 此为frame ID，标记是哪个**frame**查询过和此keyframe有相同的单词
    long unsigned int mnRelocQuery;
    // mnRelocQuery指向的Frame和此keyframe有多少共同的单词
    int mnRelocWords;
    // 当mnRelocWords大于阈值值时，就会被计算此值。
    // 此为通过dbow计算的mnRelocQuery指向的Frame与此keyframe之间**相似度的得分**
    float mRelocScore;
